...
async.waterfall([
    ...
    var query_array = result.map(function (element) {
        ...
        return [(new Date()).toISOString(), element.link, element.title, element.date, element.place];
    });
    if (err) {
    ...
    connection.query('REPLACE INTO events (parsed,link,title,date,place) VALUES ?', [query_array]);
